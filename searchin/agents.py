...
JobSearchAgent = Agent(
    role="Job Researcher",
    goal="Find recent, high-quality {title} jobs ({years} yrs experience) using public sources.",
    backstory="""
        A focused online job researcher skilled in using search APIs and platforms like Google Jobs and LinkedIn
        to find relevant opportunities for engineers.
    """,
    tools=[search_tool],
    verbose=True,
    model=openai_model
